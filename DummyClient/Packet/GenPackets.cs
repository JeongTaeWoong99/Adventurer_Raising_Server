...
public enum PacketID
{
	...
	C_EntityAttack = 1021,
	S_BroadcastEntityAttackEffectCreate = 1022,
...
public class S_BroadcastEntityAttackEffectCreate : IPacket
{
	public int ID;
	public int entityType;
	public string attackEffectSerial;
	public string attackEffectCreatePos;
	public float moveSpeed;
	public float duration;
	public string type;

	public ushort Protocol { get { return (ushort)PacketID.S_BroadcastEntityAttackEffectCreate; } }
	...
	public void Read(ArraySegment<byte> segment)
	{
		...
		ushort attackEffectCreatePosLen = BitConverter.ToUInt16(segment.Array, segment.Offset + count);
		count += sizeof(ushort);
		this.attackEffectCreatePos = Encoding.Unicode.GetString(segment.Array, segment.Offset + count, attackEffectCreatePosLen);
		count += attackEffectCreatePosLen;
	...
	public ArraySegment<byte> Write()
	{
		...
		count += sizeof(ushort);
		Array.Copy(BitConverter.GetBytes((ushort)PacketID.S_BroadcastEntityAttackEffectCreate), 0, segment.Array, segment.Offset + count, sizeof(ushort));
		count += sizeof(ushort);
		Array.Copy(BitConverter.GetBytes(this.ID), 0, segment.Array, segment.Offset + count, sizeof(int));
		count += sizeof(int);
		Array.Copy(BitConverter.GetBytes(this.entityType), 0, segment.Array, segment.Offset + count, sizeof(int));
		...
		count += sizeof(ushort);
		count += attackEffectSerialLen;
		ushort attackEffectCreatePosLen = (ushort)Encoding.Unicode.GetBytes(this.attackEffectCreatePos, 0, this.attackEffectCreatePos.Length, segment.Array, segment.Offset + count + sizeof(ushort));
		Array.Copy(BitConverter.GetBytes(attackEffectCreatePosLen), 0, segment.Array, segment.Offset + count, sizeof(ushort));
		count += sizeof(ushort);
		count += attackEffectCreatePosLen;
...
public class S_BroadcastEntityAttackResult : IPacket
{
	...
	public void Read(ArraySegment<byte> segment)
	{
		...
		count += sizeof(int);
		ushort hitEffectSerialLen = BitConverter.ToUInt16(segment.Array, segment.Offset + count);
		count += sizeof(ushort);
		this.hitEffectSerial = Encoding.Unicode.GetString(segment.Array, segment.Offset + count, hitEffectSerialLen);
		count += hitEffectSerialLen;
		this.entitys.Clear();
		ushort entityLen = BitConverter.ToUInt16(segment.Array, segment.Offset + count);
		count += sizeof(ushort);
		for (int i = 0; i < entityLen; i++)
	...
	public ArraySegment<byte> Write()
	{
		...
		count += sizeof(int);
		ushort hitEffectSerialLen = (ushort)Encoding.Unicode.GetBytes(this.hitEffectSerial, 0, this.hitEffectSerial.Length, segment.Array, segment.Offset + count + sizeof(ushort));
		Array.Copy(BitConverter.GetBytes(hitEffectSerialLen), 0, segment.Array, segment.Offset + count, sizeof(ushort));
		count += sizeof(ushort);
		count += hitEffectSerialLen;
		Array.Copy(BitConverter.GetBytes((ushort)this.entitys.Count), 0, segment.Array, segment.Offset + count, sizeof(ushort));
		count += sizeof(ushort);
...
public class C_Chatting : IPacket
{
	public string contents;

	public ushort Protocol { get { return (ushort)PacketID.C_Chatting; } }
	...
	public void Read(ArraySegment<byte> segment)
	{
		...
		count += sizeof(ushort);
		count += sizeof(ushort);
		ushort contentsLen = BitConverter.ToUInt16(segment.Array, segment.Offset + count);
		count += sizeof(ushort);
		this.contents = Encoding.Unicode.GetString(segment.Array, segment.Offset + count, contentsLen);
		count += contentsLen;
	}
	...
	public ArraySegment<byte> Write()
	{
		...
		count += sizeof(ushort);
		Array.Copy(BitConverter.GetBytes((ushort)PacketID.C_Chatting), 0, segment.Array, segment.Offset + count, sizeof(ushort));
		count += sizeof(ushort);
		ushort contentsLen = (ushort)Encoding.Unicode.GetBytes(this.contents, 0, this.contents.Length, segment.Array, segment.Offset + count + sizeof(ushort));
		Array.Copy(BitConverter.GetBytes(contentsLen), 0, segment.Array, segment.Offset + count, sizeof(ushort));
		count += sizeof(ushort);
		count += contentsLen;
...
public class S_BroadcastChatting : IPacket
{
	public int ID;
	public string contents;

	public ushort Protocol { get { return (ushort)PacketID.S_BroadcastChatting; } }
	...
	public ArraySegment<byte> Write()
	{
		...
		count += sizeof(ushort);
		Array.Copy(BitConverter.GetBytes((ushort)PacketID.S_BroadcastChatting), 0, segment.Array, segment.Offset + count, sizeof(ushort));
		count += sizeof(ushort);
		Array.Copy(BitConverter.GetBytes(this.ID), 0, segment.Array, segment.Offset + count, sizeof(int));
		count += sizeof(int);
		ushort contentsLen = (ushort)Encoding.Unicode.GetBytes(this.contents, 0, this.contents.Length, segment.Array, segment.Offset + count + sizeof(ushort));
